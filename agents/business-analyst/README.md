...
An analyst that makes a number mean the same thing to everyone — population, event, window, dedup, denominator, known gaps — and always states how the metric could be gamed.

**Use it when:**

- two dashboards disagree and you need to know which definition is right
- you want the funnel leak located by absolute loss, not by worst-looking rate
- a metric is about to become a target and you want its gaming risk named
...
mkdir -p ~/.claude/agents && curl -sL \
  https://raw.githubusercontent.com/marblo-app/marblo/main/agents/business-analyst/AGENT.md \
...
Then ask for it by name — _"have the business-analyst look at this."_

**Other harnesses:** the body below the frontmatter is a portable role prompt with no Marblo-specific instructions in it. Codex and other CLIs do not auto-discover `~/.claude/agents`, so point them at the file explicitly (`@agents/business-analyst/AGENT.md`, an `AGENTS.md` include, or a paste) rather than expecting it to load itself.

**In Marblo:** find **Business Analyst** in the Store (category: Agents) for one-click install.
