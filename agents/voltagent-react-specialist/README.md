# React Specialist (agent · community)

**Why it's here:** the registry had no frontend framework specialist at all. This one is scoped to existing React codebases rather than greenfield scaffolding — the optimisation and re-architecture end, which is where most React work actually happens.
...
mkdir -p ~/.claude/agents && curl -sL \
  "https://raw.githubusercontent.com/VoltAgent/awesome-claude-code-subagents/$SHA/categories/02-language-specialists/react-specialist.md" \
  -o ~/.claude/agents/react-specialist.md
```

Ask for it as **`react-specialist`** — VoltAgent does not namespace its frontmatter names, so the invocation name matches the filename.

## Details

- **Upstream:** [`categories/02-language-specialists/react-specialist.md`](https://github.com/VoltAgent/awesome-claude-code-subagents/blob/main/categories/02-language-specialists/react-specialist.md) · **Manifest:** [`marblo.yaml`](marblo.yaml)
