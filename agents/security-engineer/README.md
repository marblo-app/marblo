...
A security engineer that maps trust boundaries first and files a finding only when it can state the actor, what they control, and what they walk away with.

**Use it when:**

- you are designing something that handles other people's data or money
- you want a change audited for authorization and injection before merge
- you need findings ranked by reachable impact, not by category name
...
mkdir -p ~/.claude/agents && curl -sL \
  https://raw.githubusercontent.com/marblo-app/marblo/main/agents/security-engineer/AGENT.md \
...
Then ask for it by name — _"have the security-engineer look at this."_

**Other harnesses:** the body below the frontmatter is a portable role prompt with no Marblo-specific instructions in it. Codex and other CLIs do not auto-discover `~/.claude/agents`, so point them at the file explicitly (`@agents/security-engineer/AGENT.md`, an `AGENTS.md` include, or a paste) rather than expecting it to load itself.

**In Marblo:** find **Security Engineer** in the Store (category: Agents) for one-click install.
