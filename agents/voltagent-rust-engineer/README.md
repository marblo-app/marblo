# Rust Engineer (agent · community)

**Why it's here:** Rust's difficulty is concentrated in decisions a model can genuinely help with — how to shape ownership so the borrow checker stops arguing. Its prompt names `unsafe` usage as something to review, which is the right instinct.
...
mkdir -p ~/.claude/agents && curl -sL \
  "https://raw.githubusercontent.com/VoltAgent/awesome-claude-code-subagents/$SHA/categories/02-language-specialists/rust-engineer.md" \
  -o ~/.claude/agents/rust-engineer.md
```

Ask for it as **`rust-engineer`** — VoltAgent does not namespace its frontmatter names, so the invocation name matches the filename.

## Details

- **Upstream:** [`categories/02-language-specialists/rust-engineer.md`](https://github.com/VoltAgent/awesome-claude-code-subagents/blob/main/categories/02-language-specialists/rust-engineer.md) · **Manifest:** [`marblo.yaml`](marblo.yaml)
