# Python Pro (agent · community)

**Why it's here:** Python is the second language in most AI-adjacent codebases and the registry had no agent for it. This one leads with type coverage and existing project conventions rather than writing whatever it likes.
...
mkdir -p ~/.claude/agents && curl -sL \
  "https://raw.githubusercontent.com/VoltAgent/awesome-claude-code-subagents/$SHA/categories/02-language-specialists/python-pro.md" \
  -o ~/.claude/agents/python-pro.md
```

Ask for it as **`python-pro`** — VoltAgent does not namespace its frontmatter names, so the invocation name matches the filename.

## Details

- **Upstream:** [`categories/02-language-specialists/python-pro.md`](https://github.com/VoltAgent/awesome-claude-code-subagents/blob/main/categories/02-language-specialists/python-pro.md) · **Manifest:** [`marblo.yaml`](marblo.yaml)
